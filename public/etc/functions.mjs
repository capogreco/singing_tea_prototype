...
const freq_vals = bab_const.freq_vals_1
const two_pi = bab_const.two_pi

const { abs, ceil, floor, random, sqrt, PI:pi, sign } = Math

export function choose (val, left, right) {
  return right + val*(left - right)
}
...
export function formant (phasor, harm, pm_index, amp) {
    const phase_mod = sine (phasor * two_pi)* pm_index
    const [ harm_even_res, harm_even_distance ] = nearest_even (harm)
    const [ harm_odd_res, ] = nearest_odd (harm)
    const sig_even = sine (phasor * two_pi * harm_even_res + phase_mod)
    const sig_odd = sine (phasor * two_pi * harm_odd_res + phase_mod)
    const sig = cross_fade (sig_even, sig_odd, harm_even_distance * 2 - 1) * amp
    return sig
}

function nearest_even (val) {
    const res = choose (val % 2.0 < 1.0, floor (val), ceil (val))
    const distance = abs (val - res)
    return [ res, distance ]
}

function nearest_odd (val) {
    const res = choose ((val + 1.0) % 2.0 < 1.0, floor (val), ceil (val))
    const distance = abs (val - res)
    return [ res, distance ]
}

function cross_fade (left, right, mix) {
    const amp = mix * 0.5 + 0.5;
    const sig_L = sqrt (1 - amp) * left
    const sig_R = sqrt (amp) * right
    return sig_L + sig_R
}

export function table_locate (freq) {
    const too_low  = freq < freq_vals[0]
    const too_high = freq >= freq_vals[6]

    if (too_low || too_high) {
        return false
    } else {
        for (let i = 0; i < 6; ++i) {
            if (freq < freq_vals [i + 1]) {
                return evaluate_location (freq, i)
            }
        }
    }
}

function evaluate_location (freq, i) {
        const lower_val = freq_vals [i]
        const upper_val = freq_vals [i + 1]
        const difference = upper_val - lower_val
        const phase = (freq - lower_val) / difference
        return i + phase
    }

export function table_evaluate (location, formant, mode) {
    const lower_val = bab_const.table [mode][formant][floor (location)]
    const upper_val = bab_const.table [mode][formant][ceil (location)]
    const difference = upper_val - lower_val
    const phase = location % 1
    return lower_val + (difference * phase)
}
...
// from https://gist.github.com/alexkg/15258f06c0c41198d40f0a38a376bb8b

// const cos = f__cos_1vf__1f__order_4;
export const sine = f__sin_1vf__1f__order_5;

function f__cos_1vf__1f__order_4 (t) {
    t *= 1/2/pi;
    t -= floor (t);
    return f__cos_1vf__1quf__order_4 (t);
}

// export function sine (t) {
function f__sin_1vf__1f__order_5 (t) {
    t *= 1/2/pi;
    t -= floor (t);
    return f__sin_1vf__1quf__order_5 (t);
}

function f__cos_1vf__1quf__order_4 (t) {
    t *= 2;
    const u = 1 < t;
    const w = 2*u;
    t -= w;
    return f__cos_1vf__1qvf__order_4 (t);
}
function f__sin_1vf__1quf__order_5 (t) {
    t *= 2;
    const u = 1 < t;
    const w = 2*u;
    t -= w;
    return f__sin_1vf__1qvf__order_5 (t);
}

function f__cos_1vf__1qvf__order_4 (t) {
    t *= 2;
    const u = 1 < abs (t);
    const w = 2*u;
    t -= w*sign (t);
    return f__cos_1vf__1vf__order_4 (t) * (1 - w);
}
function f__sin_1vf__1qvf__order_5 (t) {
    t *= 2;
    const u = 1 < abs (t);
    const w = 2*u;
    t -= w*sign (t);
    return f__sin_1vf__1vf__order_5 (t) * (1 - w);
}

function f__cos_1vf__1vf__order_4 (t) {
    const tt = t*t;
    const a0 = 1;
    const a1 = 2 - pi*0.25;
    const a2 = 1 - pi*0.25;
    const a_ = (a0 - tt*(a1 - tt*(a2)));
    return a_;
}
function f__sin_1vf__1vf__order_5 (t) {
    const tt = t*t;
    const a0 = pi*0.5;
    const a1 = pi - 2.5;
    const a2 = pi*0.5 - 1.5;
